...
- **Исключение вопросов из распознавания**: Вопросы типа "Место 23 свободно?" больше не распознаются как объявления о свободном месте
- **Тест для вопросов**: Добавлен тест `test_check_parking_message_questions()` для проверки корректного исключения вопросов

## [1.1.0] - 2025-11-18

### Добавлено

- **Улучшенное распознавание отрицательных формулировок**: Добавлена проверка на отрицательные формулировки ("не свободно", "занято", "не освободилось") перед созданием розыгрыша
- **Тест для отрицательных формулировок**: Добавлен тест `test_check_parking_message_negative()` для проверки корректного распознавания отрицательных сообщений
- **Уведомление о свободном месте**: Если розыгрыш завершился без участников, бот отправляет сообщение "ℹ️ Место №X все еще свободно"

### Изменено

- Улучшена логика распознавания сообщений: отрицательные формулировки проверяются с приоритетом перед положительными

### Удалено

- **Сообщение-заглушка**: Убрано промежуточное сообщение "⏳ Проверяю сообщение..." - теперь сообщение с розыгрышем отправляется сразу после проверки
